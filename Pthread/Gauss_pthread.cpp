...
void calculate_se()
{
    ...
    for (int i = 0; i < THREAD_NUM; i++)
    {
        ...
        pthread_create(&threads[i], NULL, thread_se, (void *)(&thread_param_t[i]));
    }
    for (int i = 0; i < THREAD_NUM; i++)
    {
...
void calculate_pthread_block()
{
    ...
    {
        thread_param_t[i].t_id = i;
        pthread_create(&threads[i], NULL, threadFunc_block, (void *)(&thread_param_t[i]));
    }
    for (int i = 0; i < THREAD_NUM; i++)
    {
        pthread_join(threads[i], NULL);
...
int main()
{
    ...
    cout << "pthread_se:" << time / LOOP << "ms" << endl;
    ...
    for (int i = 0; i < LOOP; i++)
    {
        init_matrix();
        gettimeofday(&start, NULL);
        calculate_pthread_block();
        gettimeofday(&end, NULL);
        time += ((end.tv_sec - start.tv_sec) * 1000000 + (end.tv_usec - start.tv_usec)) * 1.0 / 1000;
    }
    cout << "pthread_block:" << time / LOOP << "ms" << endl;
}